        ...
        {projects.map((project, index) => (
          <SwiperSlide key={index} className={`p-4 rounded-xl ${project.background}`}>
            <div className="relative w-full h-[350px] md:h-[500px] flex items-center justify-center">
              ...
              />
              {activeIndex === index && (
                <div className="absolute bottom-0 left-0 mr-12 flex items-center md:hidden">
                  <div
                    className={`absolute left-0 top-0 w-full h-60 rounded-tl-full rounded-tr-full xl:${project.circleColor}  p-10 opacity-90`}
                  ></div>
